...
=head2 Configuration File Evaluation Methods

=over 2

=item B<evaluate_as_config($private_data, key1=E<gt>$val1, key2=E<gt>$val2, ...)>

This function is similar to evaluate(). However, it treats the template
as some kind of configuration file. Here is an example how it works. Given
the template:
